...
class UR_Robot:
    def __init__(self, tcp_host_ip="192.168.92.130", tcp_port=30003, workspace_limits=None,
    ...
    def testRobot(self):
        try:
            ...
            # self.move_l([0.1, 0.1, 0.1,0 , 0, 0],0.5,0.5)
    ...
    def grasp(self, position, rpyNone, open_size=0.85, k_acc=0.8, k_vel=0.8, speed=255, force=125):
        ...
        for i in range(3):
            position[i] = min(max(position[i], self.workspace_limits[i][0]), self.workspace_limits[i][1])
        # chech grasping RPY [0.5*pi,1.5*pi]
        for i in range(3):
            if rpy[i] > np.pi:
                rpy[i] -= 2 * np.pi
            elif rpy[i] < -np.pi:
